...
Найдите ген Ensembl / транскрипт / белок по стабильному идентификатору или ген по символу; возвращает основную запись аннотации (местоположение, биотип, канонический транскрипт, описание). Args:query (Ensembl stable ID ENSG.../ENST.../ENSP..., версия принята); или символ гена / алии, такие как BRAF — истинные стабильные идентификаторы &#91;ENS + необязательный код вида + буква функции + > = 6-значный блок или LRG_N&#93; маршрут к конечной точке идентификатора; Все остальное, в т.ч. Символы, начинающиеся с "ENS" как ENSA, к конечной точке символа; вид (название вида Ensembl для поиска символов, homo_sapiens по умолчанию); - проигнорированы для стабильных идентификаторов; расширение (включает дерево признаков ребенка — транскрипты/эксоны/перевод гена 's); выключено по умолчанию. Возвращает &#123;found, запрос, вид, запись &#125;; Запись не имеет значения, когда ничего не совпадает, кроме дикта для поиска вверх по течению - для гена &#123;id, display_name, описания, биотипа, object_type, seq_region_name, начала, конца, нити, assembly_name, canonical_transcript, версии, ... &#125; с интегрированными координатами на основе 1.
...
Предсказать последствия варианта с Ensembl VEP — самое тяжелое первое резюме (часто огромное) по транскрипту списка последствий. Передайте EITHER variant_id или регион + аллель. Args: variant_id (dbSNP rsID rs7412, COSMIC COSV... или HGMD ID); регион (GRCh38 1-based inclusive chrom:start-end, например). 7:140753336-140753336; SNV start==end; начальный уровень = конец + 1; суффикс с явной нитью: 1/:-1 принимается; аллель (вариантный аллель на передней пряди для маршрута по региону, например). T или - для удаления; разновидностей (по умолчанию homo_sapiens); max_consequences (капка на возвращаемых строках по транскрипту, 25 по умолчанию); полный счет в n_transcript_consequences, строки сохранены наиболее тяжелые HIGH>MODERATE>LOW>MODIFIER; transcript_consequences_truncated флаги шапки. Возвращает &#123; запрос, n_результаты, результаты:&#91;&#123;input, assembly_name, seq_region_name, начало, конец, нить, allele_string, most_severe_consequence, гены:&#91;&#123;gene_id, gene_symbol, worst_impact, n_transcripts&#125;&#93;, n_transcript_consequences, transcript_consequences_truncated, transcript_consequences:&#91;...&#93;, n_regulatory_feature_consequences, n_motif_feature_consequences, colocated_variants:&#91;...&#93;&#125;&#93;&#125;. Неизвестные rsID поднимаются вместе с сообщением вверх по течению.
...
Приведем последовательность из Ensembl — по стабильному идентификатору (ген/транскрипт/белок) или по геномной области. Передайте EITHER stable_id или регион. Args: stable_id (ENSG.../ENST.../ENSP..., версия принята); регион (включающий хром на основе 1:start..end или chrom:start-end, GRCh38 для человека, максимум 10Mb); виды (для маршрута региона, по умолчанию homo_sapiens); - проигнорированы для стабильных идентификаторов; seq_type (ID маршрут: геномный дефолт/cdna/cds/белок — белок только для ENST/ENSP); игнорируются для регионов, которые всегда возвращаются геномными; max_bytes (защита полезной нагрузки по умолчанию 400000 — более крупные последовательности `seq` опущены); длина/ша256/метадата всегда возвращается; Повторный вызов с большим max_bytes для полного текста. Возвращает &#123;found, запрос, seq_type, идентификатор, описание, молекулу, длину, sha256, seq&#125; длина в единице, подразумеваемой молекулой (основы для ДНК, остатки для белка); сек, заменяемый seq_omitted, когда он ограничен; найдено: ложное с нулевыми полями для неизвестных стабильных идентификаторов; Неблагополучные/негабаритные регионы поднимаются вместе с сообщением выше по течению.
...
Эволюционное резюме сохранения для региона из треков UCSC phyloP / phastCons (базовые оценки по многовидовым выравниваниям). Args: chrom (chr-prefixed); старт (полуоткрытый на основе 0); конечный (исключительный); пролет, ограниченный на 100000 bp — раздвоение больше; геном (по умолчанию hg38); трек (по умолчанию phyloP100way); положительный = сохраненный, отрицательный = быстро развивающийся; альтернативные варианты hg38 phastCons100way, phyloP30way, phastCons30way, phyloP447way, phyloP470way; hg19 phyloP100wayAll/phastCons100way; include_values (также возврат на базу &#123;start,end,value&#125;) строки, ограниченные max_values, values_truncated флаги шапки; По умолчанию false = summary only; max_values (по умолчанию 2000). Возвращает &#123;genome, трек, хром, начало, конец, span_bp, n_bases_covered, coverage_fraction, среднее, мин, max&#125; (+ значения, values_truncated по запросу). Статистика, взвешенная по каждому базовому пролету строки 's, подрезана к окну; Необнаруженные базы ниже coverage_fraction, не нулевые. Непрофильные трассы поднимают; Также поднимается укороченный вверх по течению список строк.
...
Совпадение пациентов. Неправильно проводить судебные разбирательства, если статус не установлен. min_age/max_age - это возраст PATIENT' (" 65 Years", " 6 Months") и совпадения испытаний, возрастное окно которых допускает пациента; Испытания на соответствие пола, признающие этот секс или всех желающих; eligibility_keywords выполняет поиск по тексту критериев включения/исключения (например, по запросу «включение/исключение»). "HbA1c > 8", "BRCA мутация", "ECOG 0-1". Требуется хотя бы одно из состояний, eligibility_keywords, min_age, max_age или пол. Страница с page_token.
...
СТРОИТЕЛЬНАЯ сеть взаимодействия белка и белка для списка генов (v12.0) при пороге доверия. Сначала отображаются символы (без карты сообщается), затем извлекаются узлы, края, резюме и происхождение.
...
Частота мутаций одного гена в нескольких исследованиях cBioPortal (1-12): мутировавшая выборка фракции секвенированной когорты в исследовании, занимающая первое место наиболее часто.
