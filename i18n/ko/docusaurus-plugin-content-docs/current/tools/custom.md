...
`host.mcp`을 통해 비즈니스 도구로 `tools/list` 프로토콜을 호출하지 마십시오; 이미 연결 중에 서버의 도구를 발견합니다. 시도 `gse60450-qc/tools/list` 호출은 알 수없는 도구로 거부되었습니다. 발견 된 작동 이름을 사용하거나 서버의 자신의 스키마를 검사합니다.
