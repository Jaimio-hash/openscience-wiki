...
**.science** 연구 패키지는 대화 지점, 파일 및 기록 된 증거를 복부에 제공합니다. colleague는 프로젝트로 가져올 수 있으며 연구 기록을 검사합니다. 수입된 회의는 읽기 전용입니다; 새로운 연구는 정상 회의에서 시작합니다.
...
가져온 세션은 열람하고 참조할 수 있지만 대화를 이어 가거나 실행할 수는 없습니다. 새로운 작업을 수행하려면 일반 세션을 만들고 명시적으로 관련 입력 파일 및 지침을 제공합니다. 수입된 사용은 지역 활동 합계에서 제외됩니다.
