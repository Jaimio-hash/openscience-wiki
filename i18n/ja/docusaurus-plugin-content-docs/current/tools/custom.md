...
`host.mcp`を介したビジネスツールとしてプロトコル`tools/list`を呼び出しないでください。 アプリケーションは、接続中にサーバーのツールを既に発見しています。 `gse60450-qc/tools/list` の試みは未知のツールとして拒否されました。 発見された操作名を使用して、またはサーバー自身のスキーマを調べます。
