...
Skills、スペシャリスト、Connectorテンプレートは異なるパッケージ境界を持っています。 フィールドを一元化し、予算をインポートします。 制限と検証ルールは、ソース検証です。 特定のアーカイブをインポートすることは、インストールされているように処理する前に、プレビューでチェックする必要があります。
