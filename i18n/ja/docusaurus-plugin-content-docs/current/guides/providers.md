...
サービスでキーを変更した後、**Settings → Model**のプロバイダを見つけて、**Edit**を選択し、**API key**の置換を入力し、保存します。 このフィールドを離れるブランクは、既存のキーを保持します。 それはそれをクリアしません。 接続テストを待ちます。 認証が失敗した場合は、エンドポイント、キーが属するアカウント、および再試行前の有効性を確認してください。
