...
**.science 研究包** 将会话分支、文件和已记录的证据放在一起，供同事交接与查看。接收方可以将其导入项目，检查研究记录。导入会话为只读；新的研究工作需要在普通会话中开展。
...
导入会话可以查看和引用，不能继续对话或执行。需要开展新工作时，建立普通会话，并明确提供相关输入文件与操作要求。导入的用量不会计入本机活动总量。
