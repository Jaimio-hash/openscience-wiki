// Adapted from @easyops-cn/docusaurus-search-local (MIT). Retains its index, highlighting and locale handling.
...
function SearchPageContent() {
    ...
    useEffect(() => {
        ...
        if (searchQuery) {
            (async () => {
                ...
                // A heading and its body can both match the same destination.
                // Keep a single entry, preferring the useful body excerpt.
                const unique = new Map();
                for (const result of results) {
                    const key = result.document.u + (result.document.h || '');
                    const existing = unique.get(key);
                    if (!existing || result.type === SearchDocumentType.Content) unique.set(key, result);
                }
                if (!cancelled) setSearchResults([...unique.values()]);
            })();
        }
        else {
    ...
    return (<React.Fragment>
      ...
      <div className="aipoch-search-page">
        <p className="aipoch-eyebrow">{translate({id:'docs.ui.searchIndex', message:'Open-Science / Documentation index'})}</p>
        <h1>{pageTitle}</h1>
        ...
        <section>
          {searchResults &&
            searchResults.slice(0, visibleCount).map((item) => (<SearchResultItem key={item.document.i} searchResult={item}/>))}
        </section>
        {searchResults?.length > visibleCount && <button className="aipoch-search-more"
          onClick={() => setVisibleCount((count) => count + 20)}>
          {translate({id:'docs.ui.moreResults', message:'Show more results'})} ↓
        </button>}
      </div>
    </React.Fragment>);
}
